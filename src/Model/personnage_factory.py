...
class Barbare(PNJ):
    def __init__(self, nom="Haroun"):
        super().__init__(nom, competence1="Tacle lourd", competence2="Codage intense", PV_max=200, current_PV=200, status=None)
        self.offensif = 35
        self.defensif = 20
    ...
    def attaquer(self, cible):
        ...
        cible.current_PV -= degats_totaux
        print(f"{self.nom} frappe avec {self.armes} ! ({degats_totaux} dégâts)")

    def defense(self):
            print(f"{self.nom} gonfle les pectoraux pour encaisser !")

class PNJFactory:
    @staticmethod
    def create_pnj(type_pnj: str) -> PNJ:
        type_pnj = type_pnj.lower()

        if type_pnj == "marchand":
            return Marchand()
        
        if type_pnj == "haroun" or type_pnj == "barbare":
            return Barbare(nom="Haroun")